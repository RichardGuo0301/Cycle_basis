"""
Two-Edge Blocking via L2-Norm MIQP (min ||Cm||_2²)
...
that share no common node, by finding integer correction Cm = C_oriented @ beta such that:
  ...
  - minimize  beta^T (C^T C) beta  =  ||Cm||_2²   (pure L2, cvxpy + SCIP)
...
# ==============================
# 5) MIQP: minimize beta^T (C^T C) beta  =  ||Cm||_2²
#          s.t. Cm[j1]=d1, Cm[j2]=d2, Cm = C_oriented @ beta, beta ∈ Z^k
#
# Variables: beta (k, integer)
# Objective: ||C_oriented @ beta||_2^2  (pure L2)
# ==============================

print(f"\nSolving MIQP:  min ||Cm||_2²  s.t. Cm[idx1]=d1, Cm[idx2]=d2 ...")

beta_var = cp.Variable(k, integer=True)
Cm_expr  = C_oriented @ beta_var

objective = cp.Minimize(cp.sum_squares(Cm_expr))
...
prob_miqp.solve(solver=cp.SCIP, verbose=False)

if prob_miqp.status not in ("optimal", "optimal_inaccurate"):
    raise RuntimeError(f"MIQP failed: status={prob_miqp.status}")

beta_star      = np.round(beta_var.value).astype(int)
Cm             = C_oriented @ beta_star
flow_corrected = flow_B + Cm
correction_method = "MIQP (L2: min ||Cm||_2²)"

combo_euler, combo_bal, combo_wc = _check_eulerian(flow_corrected)
delta_time_val = np.sum((np.abs(flow_corrected) - np.abs(flow_B)) * tau)
print(f"[miqp] Eulerian: {combo_euler}  (balanced={combo_bal}, weakly_connected={combo_wc})")
print(f"[miqp] ||Cm||_2²={int(Cm @ Cm)}   ||Cm||_1={int(np.sum(np.abs(Cm)))}")
print(f"[miqp] delta_T_total={delta_time_val:+.1f}s")
