...
def backtracking_euler_search(DG, source, edge_tau_dict, latency_req, node_to_idx, max_backtracks=50000):
    """
    Exhaustive DFS backtracking search for latency-feasible Euler circuit.
    
    Uses urgency-based ordering as a heuristic (tries most urgent neighbors first),
    but exhaustively explores all branches via backtracking until finding a feasible path.
    ...
    Args:
        ...
        max_backtracks: maximum number of backtrack attempts before timeout
    
    Returns:
        list of (u,v) tuples forming complete Euler circuit, or None if exhausted/timeout
    """
    # Build adjacency list with edge keys for tracking
    remaining = {}
    for u in DG.nodes():
        remaining[u] = []
    ...
    backtrack_count = [0]  # Use list to allow mutation in nested function

    def violates(time_now):
        """Return True if any node exceeds its allowed revisit latency at time_now."""
        for v in all_nodes:
            # If v is missing from node_to_idx for any reason, skip latency check.
            if v not in node_to_idx:
                continue
            req = float(latency_req[node_to_idx[v]])
            if not np.isfinite(req):
                continue
            if time_now - last_visit_time[v] > req:
                return True
        return False
    ...
    def dfs(current, time):
        ...
        Returns True if found complete path, False otherwise.
        """
        ...
        # Sort neighbors by urgency (most urgent first)
        # Urgency = how long neighbor has been waiting / its latency requirement
        def urgency_score(v_key_pair):
            v, _ = v_key_pair
            travel_time = edge_tau_dict.get((current, v), 1.0)
            new_time = time + travel_time
            wait = new_time - last_visit_time[v]
            return -wait / max(latency_req[node_to_idx[v]], 1e-6)  # Negative for descending sort
        
        sorted_neighbors = sorted(neighbors, key=urgency_score)
        
        # Try each neighbor in urgency order
        for next_v, edge_key in sorted_neighbors:
            travel_time = edge_tau_dict.get((current, next_v), 1.0)
            new_time = time + travel_time
            
            # Make move
            path.append((current, next_v))
            remaining[current].remove((next_v, edge_key))
            old_visit_time = last_visit_time[next_v]
            last_visit_time[next_v] = new_time
            
            # Prune immediately if this move makes ANY node violate its latency bound
            if violates(new_time):
                path.pop()
                remaining[current].append((next_v, edge_key))
                last_visit_time[next_v] = old_visit_time
                backtrack_count[0] += 1
                continue
    ...
    if success:
        print(f"  [backtrack] Found solution with {backtrack_count[0]} backtracks")
        return path
    elif backtrack_count[0] > max_backtracks:
        print(f"  [backtrack] TIMEOUT after {max_backtracks} backtracks")
        return None
    else:
        print(f"  [backtrack] EXHAUSTED search space ({backtrack_count[0]} backtracks) - no feasible path")
        return None
