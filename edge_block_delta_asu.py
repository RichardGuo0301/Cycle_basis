...
# ==============================
# 1) Graph + Basis — ASU Polytechnic Campus
# ==============================
def build_asu_graph():
    ...
    def in_preserve_box(lat, lon):
        s, n, w, e = preserve_bbox
        return (s <= lat <= n) and (w <= lon <= e)
    while True:
        to_rm = []
        for n_node, d in G_undirected.degree():
            if d < 2:
                nd = G_undirected.nodes[n_node]
                lat = nd.get('y', nd.get('lat', 0))
                lon = nd.get('x', nd.get('lon', 0))
                if not in_preserve_box(lat, lon):
                    to_rm.append(n_node)
        for n_node in list(G_undirected.nodes()):
            if n_node not in to_rm:
                nd = G_undirected.nodes[n_node]
                lat = nd.get('y', nd.get('lat', 0))
                lon = nd.get('x', nd.get('lon', 0))
...
# Node reward r: based on graph centrality
centrality = nx.degree_centrality(G_undirected)
cent_vals = np.array([centrality[nd] for nd in nodes])
# Map centrality (low -> high) to reward r (1 -> 10)
c_min, c_max = cent_vals.min(), cent_vals.max()
r = 1.0 + 9.0 * (cent_vals - c_min) / (c_max - c_min + 1e-9)
r = np.round(r, 0).astype(int)

T_max = int(sum(tau) * 1.5)   # generous upper bound on tour time
LATENCY_MIN = 700.0
LATENCY_MAX = 1000.0
MAX_SOURCE_TRIALS = n

# Latency requirements based on reward: smaller latency for higher reward
