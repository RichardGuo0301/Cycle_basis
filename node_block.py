"""
Node Blocking via Minimum-Norm Cycle Correction

Given a solved flow_B = C_oriented @ w_signed, randomly block one active node v
by finding integer correction Cm = C_oriented @ beta such that:
  - (flow_B + Cm)[j] = 0  for ALL edges j incident to v
  - minimize ||Cm||_1 subject to beta integer  (MILP)
"""
...
# ==============================
# 4) Randomly select blocked node
# ==============================
np.random.seed(7)
active_nodes = set()
for i in range(m):
...
blocked_node = int(np.random.choice(active_nodes))

# ALL incident edges of blocked_node (active + inactive)
incident_indices = [i for i in range(m) if blocked_node in edge_list[i]]
d_vec = {i: int(-flow_B[i]) for i in incident_indices}

print(f"\nRandomly blocked node: {blocked_node}")
print(f"  Incident edges ({len(incident_indices)}):")
for i in incident_indices:
    status = "ACTIVE" if flow_B[i] != 0 else "inactive"
    print(f"    idx={i}  edge={edge_list[i]}  flow_B={flow_B[i]}  need Cm={d_vec[i]}  ({status})")


# ==============================
# 5) MILP: minimize ||Cm||_1 s.t. Cm[j] = -flow_B[j] for all j incident to blocked node
#
# For each incident edge j: C_oriented[j,:] @ beta = d_vec[j]
#   active edges: d_vec[j] = -flow_B[j] (zero out)
#   inactive edges: d_vec[j] = 0 (keep zero)
# ==============================
...
# One equality pair per incident edge
n_eq_pairs = len(incident_indices)
A_eq = np.zeros((2 * n_eq_pairs, n_block_vars))
b_eq_lo = -np.inf * np.ones(2 * n_eq_pairs)
b_eq_hi = np.zeros(2 * n_eq_pairs)

for p, idx in enumerate(incident_indices):
    c_row = C_oriented[idx, :]
    d_val = float(d_vec[idx])
    A_eq[2*p,     :n_beta] =  c_row
    A_eq[2*p + 1, :n_beta] = -c_row
    b_eq_hi[2*p]     =  d_val
    b_eq_hi[2*p + 1] = -d_val
...
print(f"\nSolving MILP for minimum-L1-norm node-blocking correction "
      f"({len(incident_indices)} edge constraints)...")
result_block = milp(
    c=c_block,
...
print(f"\nVerification (all incident edges of node {blocked_node} should be 0):")
all_ok = True
for i in incident_indices:
    ok = flow_corrected[i] == 0
    all_ok = all_ok and ok
    print(f"  edge {edge_list[i]} (idx={i}): flow_corrected={flow_corrected[i]}  {'OK' if ok else 'FAIL'}")
...
print(f"  {'-'*65}")
for i in range(m):
    marker = f" ← NODE {blocked_node}" if i in incident_indices else ""
    dt = delta_time[i]
...
# ==============================
# 8) Visualization: GIF + static correction diagram
# ==============================
print("\n" + "=" * 60)
...
make_gif(list(euler_path_original), list(euler_path_original), 1,
         "visuals/node_block_original_1cycle.gif",
...
make_gif(list(euler_path_original) + list(euler_path_original), list(euler_path_original), 2,
         "visuals/node_block_original_2cycle.gif",
         G_undirected, pos, edge_tau_dict, nodes, node_to_idx, latency_requirements,
         title_prefix="Original flow_B — ")

# Static correction diagram (node blocking)
plot_static_correction(
    G_undirected=G_undirected, pos=pos, edge_list=edge_list, flow_B=flow_B, Cm=Cm,
    blocked_node=blocked_node, blocked_edge=None, incident_indices=incident_indices, blocked_idx=None,
    balanced=balanced, is_weakly_conn=is_weakly_conn, nodes=nodes,
    output_file="visuals/node_block_correction.png"
)
